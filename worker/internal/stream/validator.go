...
func (r *responseLimitReader) Read(buffer []byte) (int, error) {
	if r.remaining > 0 {
	...
	if count > 0 {
		return 0, fmt.Errorf("stream response exceeds %d bytes", r.max)
	}
	return 0, err
}
...
func Pump(
	...
	if options.MaxResponseBytes <= 0 {
		options.MaxResponseBytes = 64 << 20
	}
	if options.FirstByte <= 0 {
		options.FirstByte = 600 * time.Second
	}
	if options.Idle <= 0 {
	...
	reader := &responseLimitReader{
		reader:    body,
		remaining: options.MaxResponseBytes,
		max:       options.MaxResponseBytes,
	}
	go scan(reader, options.MaxEventBytes, events, readErrors)

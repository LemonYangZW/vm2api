...
import (
	...
	kinident "github.com/dofastted/kin-gateway/worker/internal/identity"
	...
	"github.com/dofastted/kin-gateway/worker/internal/version"
)
...
func (s *Server) Handler() http.Handler {
	...
	mux.HandleFunc("GET /internal/identity", s.authorize(s.identity))
	...
	mux.HandleFunc("POST /internal/v1/messages/count_tokens", s.authorize(s.countTokens))
...
func (s *Server) health(writer http.ResponseWriter, _ *http.Request) {
	...
	writeJSON(writer, http.StatusOK, map[string]any{
		...
		"last_error":       s.reportedLastError(current),
		"last_error_class": classifyRefreshError(s.reportedLastError(current)),
		"worker_version":   version.Version,
		"runtime_kind":     s.Config.RuntimeKind,
	})
}

func (s *Server) identity(writer http.ResponseWriter, _ *http.Request) {
	writeJSON(writer, http.StatusOK, map[string]any{
		"ok":       true,
		"identity": kinident.Collect(s.Config.RuntimeKind, version.Version),
	})
}
...
func (s *Server) credentialImport(writer http.ResponseWriter, request *http.Request) {
	writeWorkerError(
		writer,
		http.StatusGone,
		"credential_owner_moved",
		errors.New("credential import is owned by the host; slot Import is disabled"),
	)
}
...
func (s *Server) credentialEnsure(writer http.ResponseWriter, request *http.Request) {
	writeWorkerError(
		...
		http.StatusGone,
		"credential_owner_moved",
		errors.New("credential refresh is owned by the host; slot Ensure is disabled"),
	)
}
...
func (s *Server) countTokens(writer http.ResponseWriter, request *http.Request) {
	var input messageRequest
	decoder := json.NewDecoder(io.LimitReader(request.Body, s.Config.MaxRequestBytes))
	if err := decoder.Decode(&input); err != nil {
		writeWorkerError(writer, http.StatusBadRequest, "worker_request_invalid", err)
	...
	response, err := s.Upstream.CountTokens(request.Context(), input.Body, input.Headers)
	if err != nil {
		s.setLastError(err)
		writeWorkerError(writer, http.StatusBadGateway, "upstream_transport_error", err)
		return
...
func (s *Server) realtimeStream(writer http.ResponseWriter, ctx context.Context, response *upstream.Response) {
	...
	// Commit HTTP headers as soon as Anthropic does (sub2api). Waiting for the
	// first SSE event here made Node/nginx 504 during long thinking.
	writer.WriteHeader(response.Status)
	...
	if err != nil {
		writeRealtimeStreamIncomplete(writer, result, err)
		return
	}
	writer.Header().Set("X-Kin-Terminal-State", "verified")
	writer.Header().Set("X-Kin-Event-Count", fmt.Sprint(result.EventCount))
...
func writeRealtimeStreamIncomplete(writer http.ResponseWriter, result kinstream.Result, err error) {
...
func (s *Server) usage(writer http.ResponseWriter, request *http.Request) {
	if current, err := s.Store.Status(); err == nil && current.IsAPIKey() {
		writeWorkerError(writer, http.StatusBadRequest, "usage_unsupported", errors.New("console API key cannot call /api/oauth/usage"))
		return
	}
...
func (s *Server) reportedLastError(current credential.Credential) string {
	err := s.getLastError()
	if err == "" {
		return ""
	}
	if current.Valid() && !current.NeedsRefresh(time.Now(), 5*time.Minute) && classifyRefreshError(err) == "fatal" {
		s.setLastError(nil)
		return ""
	}
	return err
...
func publicCredential(current credential.Credential) map[string]any {
	...
	return map[string]any{
		"type":             credential.NormalizeType(current.Type),
		"has_access":       current.Valid(),

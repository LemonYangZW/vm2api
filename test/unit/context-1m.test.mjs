...
import {
  applyBetaPolicyToHeader,
  normalizePolicy,
  resolveContext1mPass,
  seedDefaultPolicy,
...
test('matrix pass_context_1m wins over fallback whitelist', () => {
  ...
  assert.equal(
    resolveContext1mPass('claude-sonnet-5', {
  ...
  assert.equal(
    resolveContext1mPass('claude-opus-5', {
  ...
  assert.equal(
    resolveContext1mPass('claude-sonnet-5-20260701', {
...
test('official beta policy passes 1m on sonnet-5 and filters others', () => {
  ...
  assert.match(
    applyBetaPolicyToHeader(header, 'claude-sonnet-5-20260701', { isOfficial: true }),
    /context-1m-2025-08-07/,
  )
  ...
  assert.doesNotMatch(
    applyBetaPolicyToHeader(header, 'claude-haiku-4-5-20251001', { isOfficial: true }),
    /context-1m-2025-08-07/,
  )
})
...
test('unofficial mimic never keeps context-1m even on sonnet-5', () => {
  const header = `${CONTEXT_1M_BETA},oauth-2025-04-20`
  assert.doesNotMatch(
    applyBetaPolicyToHeader(header, 'claude-sonnet-5', { isOfficial: false }),
    /context-1m-2025-08-07/,
  )
})

test('inbound [1m] injects context-1m on allowed sonnet-5 for official and unofficial', () => {
  const official = applyBetaPolicyToHeader('', 'claude-sonnet-5', { isOfficial: true, want1m: true })
  assert.match(official, /context-1m-2025-08-07/)
  const unofficial = applyBetaPolicyToHeader('', 'claude-sonnet-5', { isOfficial: false, want1m: true })
  assert.match(unofficial, /context-1m-2025-08-07/)
  assert.match(unofficial, /oauth-2025-04-20/)
  assert.doesNotMatch(
    applyBetaPolicyToHeader('', 'claude-opus-5', { isOfficial: false, want1m: true }),
    /context-1m-2025-08-07/,
  )
  assert.doesNotMatch(
    applyBetaPolicyToHeader('', 'claude-sonnet-4-6', { isOfficial: true, want1m: true }),
    /context-1m-2025-08-07/,
  )
  assert.equal(ensureContext1mBeta('oauth-2025-04-20'), `${CONTEXT_1M_BETA},oauth-2025-04-20`)
  assert.equal(ensureContext1mBeta(`${CONTEXT_1M_BETA},oauth-2025-04-20`), `${CONTEXT_1M_BETA},oauth-2025-04-20`)
})

...
test('follow accepts exact canary and rejects extras', () => {
  ...
  assert.equal(
    scoreCapability(caze, { ok: true, status: 200, text: 'BEGIN-09D0CECA-END', usage: {}, stop_reason: 'end_turn' })
      .exact,
    true,
  )
  assert.equal(
    scoreCapability(caze, { ok: true, status: 200, text: 'OK BEGIN-09D0CECA-END', usage: {}, stop_reason: 'end_turn' })
      .exact,
    false,
  )
})

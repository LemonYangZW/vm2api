...
test('CRS keeps a long caller system in the structured official prompt', async () => {
  ...
  try {
    ...
    const tr = readTrace(gw)
    assert.equal(tr?.via, 'go-worker')
    assert.ok(tr.system.some((block) => block?.text === CRS_OFFICIAL_SYSTEM))
    const callerSystem = tr.system.find((block) => String(block?.text || '').startsWith('S'))
    assert.equal(callerSystem?.text.length, 30000)
  } finally {
    await gw.stop()
  }
})

...
export const DEFAULT_MODELS = ['claude-opus-5', 'claude-sonnet-5', 'claude-fable-5']
...
function writeInstruction(destPath) {
  if (!destPath) return []
  return [`- 将完整 Markdown 正文写入文件：${destPath}`, '- 只输出将写入该文件的正文，不要解释写文件过程']
}
...
function renderMarkdown(run) {
  ...
  lines.push(
    `- 结果：${sum.ok}/${sum.total} 成功（${Math.round(sum.success_rate * 100)}%） 失败 ${sum.error} 取消 ${sum.cancelled}`,
  )
  lines.push(
    `- 会话耗时 p50/p95/max：${sum.duration_ms.p50 ?? '—'} / ${sum.duration_ms.p95 ?? '—'} / ${sum.duration_ms.max ?? '—'} ms`,
  )
  ...
  for (const s of run.sessions) {
    ...
    lines.push(
      `| ${s.index + 1} | ${s.model} | ${s.stock.ticker} | ${s.status} | ${s.duration_ms || '—'} | ${s.turns.filter((t) => t.finished).length}/${s.plannedTurns} | ${String(preview).replace(/\|/g, '/').slice(0, 80)} |`,
    )
  }
  lines.push('')
  if (run.error) {
...
function publicRun(run, { includeText = false } = {}) {
  ...
  return {
    ...
    report_markdown:
      run.status === 'ok' || run.status === 'error' || run.status === 'cancelled' ? renderMarkdown(run) : null,
...
async function runSession(run, session) {
  ...
  try {
    for (let n = 1; n <= session.plannedTurns; n++) {
      ...
      const prompt =
        n === 1 ? buildReportPrompt(session.stock, destPath) : buildFollowupPrompt(session.stock, n, destPath)
...
export function startConcurrentTest(opts = {}) {
  ...
  const timeoutMs = Math.max(30000, Math.min(600000, Number(opts.timeout_ms) || Math.max(180000, maxTokens * 12)))
  ...
  if (run.dataDir) {
    try {
      ensureReportsDir(run.dataDir, run.reportDay)
    } catch {}
  }
  runs.set(id, run)
  activeId = id

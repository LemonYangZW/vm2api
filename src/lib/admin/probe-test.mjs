...
import { beginTestJob, endTestJob, getTestJob, isTestJobBusy, markTestJob, busyMessage } from './test-job-lock.mjs'
...
export function buildNeedleFiller(targetChars = 24000) {
  ...
  let chars = 0
  const lineAt = (n) =>
    `row ${String(n).padStart(4, '0')}: ledger note ${(n * 17) % 997} — placeholder inventory, no special marker.`
...
export const FORM_QUESTIONS = [
  {
    ...
    label: '定价折扣',
    gold: '86.4',
    user: '一件商品进价 80 元，按 20% 利润定价后打 9 折出售。实际售价是多少元？',
  },
  {
    id: 'q2',
    label: '相遇',
    gold: '2.571428',
    accept: ['2.57', '18/7', '2.571', '2.5714'],
    user: '甲乙两地相距 360 千米。客车时速 80，货车时速 60，同时相向开出。多少小时后相遇？',
  },
  ...
  { id: 'q15', label: '正方形', gold: '25', user: '边长 5 厘米的正方形，面积是多少平方厘米？' },
  {
    ...
    label: '分数加',
    gold: '11/12',
    accept: ['0.916666', '0.917'],
    user: '计算 3/4 + 1/6，结果写成最简分数。',
  },
...
function pickFromPool(pool, opts, defaultSample) {
  ...
  const seed =
    opts.seed == null || opts.seed === '' ? crypto.randomBytes(4).readUInt32BE(0) : Number(opts.seed) >>> 0 || 1
...
export function pickFormQuestions(opts = {}) {
  const allow = Array.isArray(opts.questions) && opts.questions.length ? new Set(opts.questions.map(String)) : null
  const pool = allow ? FORM_QUESTIONS.filter((q) => allow.has(q.id)) : [...FORM_QUESTIONS]
  return pickFromPool(pool.length ? pool : [...FORM_QUESTIONS], opts, DEFAULT_FORM_SAMPLE)
}

export function pickCapabilityCases(opts = {}) {
  const allow = Array.isArray(opts.cases) && opts.cases.length ? new Set(opts.cases.map(String)) : null
  const pool = allow ? CAPABILITY_CASES.filter((c) => allow.has(c.id)) : [...CAPABILITY_CASES]
  const source = pool.length ? pool : [...CAPABILITY_CASES]
  const hasSample = Number(opts.sample ?? opts.sample_size) > 0
  return pickFromPool(
    source,
    { ...opts, bank: CAPABILITY_CASES.length },
    hasSample ? Number(opts.sample ?? opts.sample_size) : source.length,
  )
}
...
export function scoreForm(question, kind, turns) {
  ...
  const exact =
    answersMatch(got, question.gold) ||
    (question.accept || []).some((alt) => answersMatch(got, alt)) ||
    (kind === 'mixed' &&
      (answersMatch(second.text, question.gold) ||
        (question.accept || []).some((alt) => answersMatch(second.text, alt))))
  const reasoned = kind === 'answer' ? true : hasReasoningText(kind === 'mixed' ? second.text || '' : first.text || '')
...
async function consumeMessagesResponse(res, { onProgress } = {}) {
  ...
  return {
    ...
    status,
    headers,
    body: { content: compact, usage, stop_reason: stopReason },
    text,
    thinking,
    usage,
    stop_reason: stopReason,
...
function assistantMessage(result) {
  const blocks = (result.blocks || []).filter(
    (b) => b && (b.type === 'text' || b.type === 'thinking' || b.type === 'redacted_thinking'),
  )
  if (blocks.length) {
    return {
      role: 'assistant',
...
export function buildProbeTurnRequest({ model, messages, maxTokens, sessionId, rewrite = false } = {}) {
  const caps = getCapabilities(model) || {}
  let thinking = null
  if (
    caps.requires_adaptive ||
    caps.thinking_mode === 'adaptive_only' ||
    /claude-(opus|sonnet|fable)-5/.test(model) ||
    /claude-opus-4-[5-8]/.test(model)
  ) {
    thinking = { type: 'adaptive' }
  }
  let body = claudeCodeInboundBody({ model, messages, maxTokens, thinking, sessionId, stream: true })
  if (rewrite) {
    body = applyCrsUnofficialPersona(body, {
...
async function postProbeTurn({
  ...
  const { body, headers } = buildProbeTurnRequest({ model, messages, maxTokens, sessionId, rewrite: true })
...
async function runOneCapability(run, item) {
  ...
  attachStream(
    item,
    {
      ...
      thinking_chars: String(item.thinking || '').length,
      tokens_out: Number(result.usage?.output_tokens) || 0,
...
async function runQueue(run) {
  ...
  const workers = Array.from({ length: run.concurrency }, async () => {
    while (true) {
      ...
      if (idx > 0) {
        try {
          await sleep(400, run.abort.signal)
        } catch {
          return
        }
      }
        ...
        if (!item.raw) {
          item.raw = packProbeRaw(
            { model: item.model, messages: [], stream: true },
            {
              ...
              status: 0,
              headers: {},
              body: { error: { type: 'cancelled', message: item.error } },
              events: [],
              event_count: 0,
            },
          )
...
export function startProbeTest(opts = {}) {
  ...
  const run = {
    ...
    bank:
      suite === 'forms'
        ? (built.pick?.bank ?? FORM_QUESTIONS.length)
        : (built.capPick?.bank ?? CAPABILITY_CASES.length),

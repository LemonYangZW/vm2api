...
import { isOfficialClaudeCodeTraffic, looksLikeOhMyPiSystem } from '../identity/crs-persona.mjs'
...
export function classifyClient(headers, body) {
  ...
  if (isOfficialClaudeCodeTraffic(headers, body)) return 'claude_code_official'
  if (looksLikeOhMyPiSystem(body?.system) || /oh-my-pi|\bomp\//i.test(ua)) return 'oh_my_pi'
  if (isOfficialClaudeUa(ua) || /claude-cli|claude-code/i.test(ua)) return 'third_party_sdk'

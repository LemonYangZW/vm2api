...
function officialStainlessHeaders() {
  return {
    "x-stainless-lang": WRAP_OFFICIAL_STAINLESS.lang,
    "x-stainless-os": WRAP_OFFICIAL_STAINLESS.os,
    "x-stainless-arch": WRAP_OFFICIAL_STAINLESS.arch,
    "x-stainless-runtime": WRAP_OFFICIAL_STAINLESS.runtime,
    "x-stainless-runtime-version": WRAP_OFFICIAL_STAINLESS.runtimeVersion,
    "x-stainless-package-version": WRAP_OFFICIAL_STAINLESS.packageVersion,
...
function resetOfficialH2Fetch() {
  h2Fetch = undefined;
}
function officialH2Fetch() {
  if (process.env.HTTPS_PROXY || process.env.https_proxy || process.env.HTTP_PROXY || process.env.http_proxy || process.env.ANTHROPIC_UNIX_SOCKET) {
    return;
  }
  if (h2Fetch)
    return h2Fetch;
  try {
    ...
    h2Fetch = (input, init) => undici.fetch(input, {
      ...init,
      dispatcher: agent
    });
    return h2Fetch;
...
var WRAP_OFFICIAL_STAINLESS, h2Fetch;
var init_officialFingerprint = __esm(() => {
  init_cliVersion();
  WRAP_OFFICIAL_STAINLESS = Object.freeze({
    lang: "js",
    os: "Linux",
    arch: "x64",
    runtime: "node",
...
function configureGlobalAgents() {
  axios_default.defaults.headers.common["User-Agent"] = officialCliUserAgent();
  const proxyUrl = getProxyUrl();
  const mtlsAgent = getMTLSAgent();
...
async function getAWSClientProxyConfig() {
  ...
  const [{ NodeHttpHandler }, { defaultProvider }] = await Promise.all([
    import("./chunk-vy0c1bwp.js"),
    import("./chunk-907332er.js")
  ]);
...
export { HttpsProxyAgent, init_dist2 as init_dist, getCACertificates, clearCACertsCache, init_caCerts, getMTLSConfig, getWebSocketTLSOptions, clearMTLSCache, configureGlobalMTLS, init_mtls, WRAP_OFFICIAL_CLI_VERSION, officialCliVersion, init_cliVersion, officialCliUserAgent, officialStainlessHeaders, applyOfficialOutboundHeaders, resetOfficialH2Fetch, officialH2Fetch, init_officialFingerprint, disableKeepAlive, getProxyUrl, shouldBypassProxy, createAxiosInstance, getWebSocketProxyAgent, getWebSocketProxyUrl, getProxyFetchOptions, configureGlobalAgents, getAWSClientProxyConfig, clearProxyCache, init_proxy };

//# debugId=A1AF5E19B5AAC41964756E2164756E21
//# sourceMappingURL=chunk-64er5ppc.js.map

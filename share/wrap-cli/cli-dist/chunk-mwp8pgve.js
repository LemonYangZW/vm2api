...
async function connectDoubaoStream(callbacks, _options) {
  let doubaoAsr;
  try {
    doubaoAsr = await import("./chunk-wgnyph3q.js");
  } catch (err) {
    logError(new Error(`[doubao-asr] Failed to import doubaoime-asr package: ${String(err)}`));
  ...
  (async () => {
    try {
      ...
      const gen = transcribeRealtime(audioSource, {
        config
      });
...
function useVoice({
  ...
  import_react.useEffect(() => {
    if (enabled && !voiceModule) {
      import("./chunk-xd91rdyx.js").then((mod) => {
        voiceModule = mod;
      });
    }
  }, [enabled]);
...
//# debugId=2231BC0ACDBE1C3564756E2164756E21
//# sourceMappingURL=chunk-mwp8pgve.js.map

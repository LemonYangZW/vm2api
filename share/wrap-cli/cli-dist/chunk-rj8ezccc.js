...
function toAbsolutePath(filePath, cwd) {
  if (!filePath)
    return;
  if (!cwd)
    return filePath;
...
function toolInfoFromToolUse(toolUse, _supportsTerminalOutput = false, cwd) {
  ...
  switch (name) {
    ...
    case "Read": {
      const inputFilePath = input?.file_path;
      ...
      return {
        ...
        kind: "read",
        locations: absReadPath ? [{ path: absReadPath, line: offset ?? 1 }] : [],
        content: []
      };
    }
    case "Write": {
    ...
    case "WebFetch": {
      ...
      return {
        ...
        content: fetchPrompt ? [
          {
            type: "content",
            content: { type: "text", text: fetchPrompt }
          }
        ] : []
      };
    }
...
function assistantMessageToAcpNotifications(msg, sessionId, toolUseCache, conn, options) {
  ...
  const contentToProcess = options?.streamingActive ? content.filter((block) => block.type !== "text" && block.type !== "thinking") : content;
  if (contentToProcess.length === 0)
    return [];
  return toAcpNotifications(contentToProcess, "assistant", sessionId, toolUseCache, conn, undefined, options);
}
...
function nextSdkMessageOrAbort(sdkMessages, abortSignal) {
  ...
  const abortPromise = new Promise((resolve3) => {
    abortHandler = () => resolve3({ done: true, value: undefined });
...
Compacting completed.` }
        ...
        case "result": {
          ...
          if (usage) {
            ...
            accumulatedUsage.cachedWriteTokens += usage.cache_creation_input_tokens ?? 0;
          }
        ...
        case "stream_event": {
          ...
          const streamMessageId = streamParent === null ? currentAgentMessageId ?? undefined : undefined;
        ...
        case "assistant": {
          ...
          if (assistantMsg?.usage && parentToolUseId === null) {
            ...
            lastAssistantTotalUsage = (typeof usage.input_tokens === "number" ? usage.input_tokens : 0) + (typeof usage.output_tokens === "number" ? usage.output_tokens : 0) + (typeof usage.cache_read_input_tokens === "number" ? usage.cache_read_input_tokens : 0) + (typeof usage.cache_creation_input_tokens === "number" ? usage.cache_creation_input_tokens : 0);
          }
...
async function replayHistoryMessages(sessionId, messages, conn, toolUseCache, clientCapabilities, cwd) {
  for (const rawMsg of messages) {
    ...
    if (Array.isArray(content)) {
      const replayMessageId = randomUUID();
      const notifications = toAcpNotifications(content, role, sessionId, toolUseCache, conn, undefined, { clientCapabilities, cwd, messageId: replayMessageId });
      for (const notification of notifications) {
        await conn.sessionUpdate(notification);
      }
    }
...
// src/services/acp/permissions.ts
function createAcpCanUseTool(conn, sessionId, _getCurrentMode, clientCapabilities, cwd, onModeChange, isBypassModeAvailable, onPermissionCancelled) {
  return async (tool, input, context, assistantMessage, toolUseID, forceDecision) => {
    ...
    if (tool.name === "ExitPlanMode") {
      return handleExitPlanMode(conn, sessionId, toolUseID, input, supportsTerminalOutput, cwd, onModeChange, isBypassModeAvailable, onPermissionCancelled);
    }
...
async function handleExitPlanMode(conn, sessionId, toolUseID, input, supportsTerminalOutput, cwd, onModeChange, isBypassModeAvailable, onPermissionCancelled) {
  const options = [
    {
      kind: "allow_always",
...
class AcpAgent {
  ...
  async listSessions(params) {
    if (params.cursor !== undefined && params.cursor !== null) {
      throw new Error("Pagination cursor not supported: listSessions returns all results in a single page.");
    }
...
async function loadForkSourceMessages(sessionId) {
  let initialMessages;
  try {
...
function resolveSessionPermissionMode(metaMode, hasMetaMode, settingsMode) {
  if (hasMetaMode) {
    ...
    if (metaResolved === "bypassPermissions" && !isAcpBypassPermissionModeAvailable()) {
      throw new Error("Mode not available: bypassPermissions cannot run as root (start the agent as a non-root user, or set IS_SANDBOX=1).");
    }
    return metaResolved;
  }
...
function isAcpBypassPermissionModeAvailable() {
...
function flattenConfigOptionValues(options) {
  const values = [];
...
function getConnection(agent) {
  return agent.conn;
}
function readClientCapabilities(agent) {
  return agent.clientCapabilities;
}
function syncSessionConfigState(_agent, session, configId, value) {
  if (configId === "mode") {
...
async function createSession(params, opts = {}) {
  enableConfigs();
  const sessionId = opts.sessionId ?? randomUUID2();
  const cwd = params.cwd;
  const currentProjectDir = getSessionProjectDir();
  switchSession(sessionId, currentProjectDir);
  setOriginalCwd(cwd);
  ...
  try {
    ...
    const canUseTool = createAcpCanUseTool(conn, sessionId, () => this.sessions.get(sessionId)?.modes.currentModeId ?? "default", clientCapabilities, cwd, (modeId) => {
...
async function getOrCreateSession(params) {
  ...
  if (existingSession) {
    ...
    if (fingerprint === existingSession.sessionFingerprint) {
      ...
      switchSession(params.sessionId, resolved2 ? dirname(resolved2.filePath) : null);
  ...
  switchSession(params.sessionId, projectDir);
  setOriginalCwd(params.cwd);
  let initialMessages;
  if (resolved) {
    try {
  ...
  if (shouldReplay && initialMessages && initialMessages.length > 0) {
    const session = this.sessions.get(params.sessionId);
    if (session) {
      await replayHistoryMessages(params.sessionId, initialMessages, getConnection(this), session.toolUseCache, readClientCapabilities(this), session.cwd);
    }
  }
  return {
...
async function replaySessionHistory(params) {
  try {
    const log = await getLastSessionLog(params.sessionId);
    if (!log || log.messages.length === 0)
      return;
    ...
    if (!session)
      return;
    await replayHistoryMessages(params.sessionId, messages, getConnection(this), session.toolUseCache, readClientCapabilities(this), session.cwd);
  } catch (err) {
    console.error("[ACP] Failed to replay session history:", err);
  }
}
function applySessionMode(sessionId, modeId) {
...
async function updateConfigOption(sessionId, configId, value) {
  const session = this.sessions.get(sessionId);
  if (!session)
    return;
  ...
  for (const block of prompt) {
    ...
    if (b.type === "text") {
      ...
      if (resource && typeof resource.text === "string") {
        ...
        const mt = typeof resource.mimeType === "string" ? resource.mimeType : "application/octet-stream";
        const uri = typeof resource.uri === "string" ? resource.uri : "(unknown uri)";
        parts.push(`Embedded resource: ${uri} (${mt}, base64 blob, ${resource.blob.length} chars)`);
      }
    }
  }
...
async function prompt(params) {
  ...
  if (session.promptRunning) {
    const promptUuid = randomUUID3();
    const cancelled = await new Promise((resolve3) => {
      session.pendingQueue.push(promptUuid);
      session.pendingMessages.set(promptUuid, { resolve: resolve3 });
    });
    if (cancelled) {
  ...
  try {
    ...
    switchSession(params.sessionId, getSessionProjectDir());
    const sdkMessages = session.queryEngine.submitMessage(promptInput);
    const { stopReason, usage } = await forwardSessionUpdates(params.sessionId, sdkMessages, getConnection(this), session.queryEngine.getAbortSignal(), session.toolUseCache, readClientCapabilities(this), session.cwd, () => session.cancelled);
    ...
    if (usage) {
      ...
      const usagePayload = {
        ...
        thoughtTokens,
        totalTokens: usage.inputTokens + usage.outputTokens + usage.cachedReadTokens + usage.cachedWriteTokens + thoughtTokens
      };
      return {
        stopReason,
        usage: usagePayload,
    ...
    const isAbort = err instanceof Error && (err.name === "AbortError" || /abort|cancelled|interrupt/i.test(err.message));
...
async function setSessionConfigOption(params) {
  const session = this.sessions.get(params.sessionId);
  if (!session) {
    throw new Error("Session not found");
  }
  ...
  if (option.type === "select") {
    const validValues = flattenConfigOptionValues(option.options);
    if (!validValues.includes(params.value)) {
      throw new Error(`Invalid value '${params.value}' for config option ${params.configId}; must be one of: ${validValues.join(", ")}`);
    }
  }
...
async function emitSessionInfoUpdate(agent, sessionId, firstPrompt) {
  const session = agent.sessions.get(sessionId);
  if (!session)
    return;
  const cache = session.toolUseCache;
  if (cache.__sessionInfoTitleSent)
    return;
...
//# debugId=8723F8D7A534E88964756E2164756E21
//# sourceMappingURL=chunk-rj8ezccc.js.map

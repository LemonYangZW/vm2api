...
var __dirname = "/mnt/x/kin/.ccg/claude-code-latest/src/utils/computerUse/win32", bridgeProc = null, requestId = 0, pendingRequests, outputBuffer = "";
...
function createCrossPlatformExecutor(_opts) {
  ...
  return {
    ...
    async resolvePrepareCapture(opts) {
      const d = platform4.display.getSize(opts.preferredDisplayId);
      const raw = await platform4.screenshot.captureScreen(opts.preferredDisplayId);
    ...
    async screenshot(opts) {
      const d = platform4.display.getSize(opts.displayId);
      const raw = await platform4.screenshot.captureScreen(opts.displayId);
...
$i = New-Object MUp+INPUT; $i.type=0; $i.mi.dwFlags=0x0004; [MUp]::SendInput(1, @($i), [Runtime.InteropServices.Marshal]::SizeOf($i)) | Out-Null`
    ...
    async manageWindow(action, opts) {
      if (!platform4.windowManagement)
        return false;
      const result = platform4.windowManagement.manageWindow(action, opts);
      _ncOffset = null;
      _ncOffsetHwnd = null;
      return result;
    },
        ...
        for (let r = 0;r < repeat; r++) {
          ...
          switch (opts.action) {
            case "type":
              ...
              break;
            case "combo": {
              const parts = opts.text.split("+").map((k) => k.trim()).filter(Boolean);
              wm.sendKeys(hwnd, parts);
              break;
            }
            case "press": {
              const lower = opts.text.toLowerCase();
              const vk = VK_MAP3[lower] ?? (opts.text.length === 1 ? opts.text.charCodeAt(0) : 0);
              if (vk)
      ...
      try {
        ...
        switch (opts.responseType) {
          ...
          case "escape":
            ...
            break;
          case "select": {
            const vk = (opts.arrowDirection ?? "down") === "down" ? VK_DOWN : VK_UP;
...
async function writeClipboardViaPbcopy(text) {
  if (process.platform === "win32") {
    const { code: code2 } = await execFileNoThrow("powershell", [
      "-NoProfile",
      "-Command",
...
//# debugId=FBC9DC598BAE6D1B64756E2164756E21
//# sourceMappingURL=chunk-msarpzkm.js.map

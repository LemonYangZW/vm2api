...
// packages/@ant/model-provider/src/shared/openaiUsage.ts
function normalizeOpenAIUsage(params) {
  const totalInput = Math.max(0, params.totalInputTokens);
  const cacheRead = Math.min(Math.max(0, params.cacheReadTokens ?? 0), totalInput);
  const remainingAfterRead = Math.max(0, totalInput - cacheRead);
  const cacheCreation = Math.min(Math.max(0, params.cacheWriteTokens ?? 0), remainingAfterRead);
  return {
...
var init_openaiUsage = () => {};

// packages/@ant/model-provider/src/shared/openaiStreamAdapter.ts
import { randomUUID as randomUUID2 } from "crypto";
async function* adaptOpenAIStreamToAnthropic(stream, model, options) {
  ...
  for await (const chunk of stream) {
    ...
    if (chunk.usage) {
      ...
      const details = detailsValue && typeof detailsValue === "object" ? detailsValue : undefined;
      if (typeof details?.cached_tokens === "number") {
        rawCacheReadTokens = details.cached_tokens;
      }
      if (options?.includeCacheWriteTokens && typeof details?.cache_write_tokens === "number") {
...
var init_openaiStreamAdapter = __esm(() => {
  init_openaiUsage();
});
